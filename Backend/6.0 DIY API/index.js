...
app.get("/random", (req,res)=> {
    const randomJokePos = Math.floor(Math.random() * jokes.length);
    res.json(jokes[randomJokePos]);
});

//2. GET a specific joke
app.get("/jokes/:id", (req,res)=> {
  const id = parseInt(req.params.id);
  res.json(jokes.find(item => item.id === id));
});

//3. GET a jokes by filtering on the joke type
app.get("/filter", (req,res)=> {
  res.json(jokes.filter(item => item.jokeType === req.query.type));
});
...
app.put("/jokes/:id", (req,res)=> {
  ...
  if(!joke){
    ...
    res.status(500).send("Couldn't find a joke with this id");
  }
